...
export const PokemonProvider: React.FC<{ children: React.ReactNode }> = ({ children }: { children: React.ReactNode }) => {
  ...
  // Función auxiliar para leer siempre el localStorage actual
  const obtenerMochilaGuardada = (usuarioId: number): pokemonTarjeta[] => {
    const data = localStorage.getItem(`mochila_${usuarioId}`);
    return data ? JSON.parse(data) : [];
  };

  const cargarMochilaEntrenador = (UsuarioId: number) => {
    const mochila = obtenerMochilaGuardada(UsuarioId);
    setmochilaActual(mochila);
  };
  ...
  useEffect(() => {
    ...
    const sincronizarDesdeStorage = (event?: StorageEvent) => {
      if (!event || event.key === `mochila_${EntrenadorActivo.id}` || event.key === 'entrenador_Activo_id') {
        cargarMochilaEntrenador(EntrenadorActivo.id);
      }
    };

    sincronizarDesdeStorage();
    window.addEventListener('storage', sincronizarDesdeStorage);

    return () => {
      window.removeEventListener('storage', sincronizarDesdeStorage);
    };
  }, [EntrenadorActivo]);
  ...
  const GuardarMochila = (pokemon: pokemonTarjeta) => {
    ...
    // Lee el localStorage actual para que si fue borrado en devtools no mantenga lo anterior
    const mochilaStorage = obtenerMochilaGuardada(EntrenadorActivo.id);
    const actualizada = [...mochilaStorage, { ...pokemon, esFavorito: false }];
  ...
  const actualizarFavorito = (pokemonId: number) => {
    ...
    const mochilaStorage = obtenerMochilaGuardada(EntrenadorActivo.id);
    const actualizada = mochilaStorage.map(p =>
  ...
  const eliminarpokemon = (pokemonId: number) => {
    ...
    const mochilaStorage = obtenerMochilaGuardada(EntrenadorActivo.id);
    const filtrado = mochilaStorage.filter(p => p.id !== pokemonId);
